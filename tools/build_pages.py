...
"""
import os, json
from build_area import head, footer, mailto, esc, BASE, ROOT, CASES, trust_strip, cta_dual, cta_mid, founder_note, sticky_cta, final_cta
...
def cta_block(subject, msg):
    return final_cta(subject.replace("無料簡易診断の申込（", "").replace("）", ""), msg)
...
def write_page(path, html):
    ctx = path.strip("/").replace("/", "-") or "top"
    html = html.replace('<footer class="site-foot">', sticky_cta(ctx) + '<footer class="site-foot">', 1)
...
def page_subsidy():
    ...
    faqs = [
        ...
        ("いくら戻ってきますか？", "中小企業の場合、経費助成75%（上限あり）に加えて、訓練中の賃金助成が1人1時間あたり1,000円支給されます。例えば受講料100万円・10時間の研修を5名が受けると、経費助成75万円＋賃金助成5万円で実質負担は約20万円。おおむね受講料の4分の1程度が目安です。"),
...
<section>
  ...
  <h2>負担額シミュレーター</h2>
  <p>受講料（1名あたり）と人数を入力すると、実質負担額を試算できます。どの研修会社のAI研修にもお使いいただけます。</p>
  <div class="sim" id="sim">
    ...
    <div class="sim-row">
      <label for="simPrice">受講料（税抜・1名あたり）</label>
      <input type="number" id="simPrice" value="100000" min="10000" step="10000" style="width:130px"> 円
      <span class="s-meta">※ 10時間の研修を想定</span>
    </div>
    <div class="sim-out">
      <div class="o-row"><span>受講料総額（受講料 × 人数）</span><span id="oTotal">—</span></div>
...
<section>
  ...
  {cta_mid("助成金", "自社の研修が助成金の対象になるか、無料診断であわせて確認できます。")}
...
(function(){{
  var chusho=true;
  var bC=document.getElementById('btnChusho'),bD=document.getElementById('btnDai'),n=document.getElementById('simN'),pr=document.getElementById('simPrice');
  function yen(v){{return v.toLocaleString('ja-JP')+'円'}}
  function calc(){{
    var num=Math.max(1,Math.min(50,parseInt(n.value||'1',10)));
    var price=Math.max(0,parseInt(pr.value||'0',10)),hours=10;
...
def page_training():
    ...
    desc = "非エンジニアの社員向けAI研修。教材は「構築済みの自社業務フロー」だから、翌日から仕事で使えます。4回×2.5時間の10時間カリキュラム。人材開発支援助成金対応で実質負担約1/4に。研修のみの単独依頼も可能。"
    faqs = [
        ...
        ("費用はいくらですか？助成金は使えますか？", "受講料は人数・形式（対面／オンライン）・演習内容により個別にお見積りします。人材開発支援助成金「事業展開等リスキリング支援コース」の対象になる場合、中小企業なら実質負担は受講料の約1/4になります。詳しくは助成金ページをご覧ください。"),
    ]
    ld = graph(
        {
            "@type": "Service",
...
<section>
  ...
  <h2>料金と助成金</h2>
  <p>受講料は人数・形式・演習内容に応じて個別にお見積りします。人材開発支援助成金「事業展開等リスキリング支援コース」の対象になる場合、中小企業は<b>経費助成75%＋賃金助成1,000円/時</b>が支給され、実質負担は受講料の約4分の1に。2026年度が最終年度のため、活用するなら年度内の訓練開始が確実です。</p>
  <p><a href="/subsidy/">→ 助成金の詳細と負担額シミュレーターはこちら</a></p>
</section>
{cta_mid("AI研修", "研修の内容と助成金の活用可否を、無料診断でまとめてご確認いただけます。")}
...
<div class="hero">
  ...
  {trust_strip()}
  {cta_dual('生成AI導入支援')}
  <p class="hero-meta">全国対応（東京都内は渋谷オフィスから直接訪問）</p>
</div>
...
<section>
  ...
  <div class="tbl-wrap"><table class="tbl">
    <thead><tr><th>ステップ</th><th>内容</th><th>期間・工数</th><th>費用</th></tr></thead>
    <tbody>
      <tr><td><b>STEP 0</b></td><td>無料簡易診断——業務概要を伺い、AI化の期待効果の目安をその場でご提示</td><td>オンライン・約2時間</td><td><b>無料</b></td></tr>
      <tr><td><b>STEP 1</b></td><td>実地調査（AI化アセスメント）——業務棚卸し・ロードマップ・削減効果試算の納品</td><td>30〜90時間・約1〜3ヶ月</td><td>個別お見積り</td></tr>
      <tr class="hl"><td><b>STEP 2</b></td><td>駐在型AI化支援——現場で手を動かしながら実装（現地30h＋リモート30hの月60h稼働例）</td><td>月60時間・3〜6ヶ月</td><td>個別お見積り（準委任・月額稼働型）</td></tr>
      <tr><td><b>STEP 3</b></td><td>内製化（AI研修）——構築済みの自社業務フローを教材に保守運用を習得</td><td>4回×2.5時間</td><td>個別お見積り<br><span class="s-meta">助成金で実質約1/4に</span></td></tr>
    </tbody>
  </table></div>
  <p class="s-meta">料金は無料診断・実地調査の結果に基づき、稼働時間・体制を設計して個別にお見積りします。研修費用は<a href="/subsidy/">人材開発支援助成金</a>で実質負担を大きく下げられます（2026年度が最終年度）。</p>
</section>
{founder_note()}
...
# ================================================================ /column/ ハブ
ARTICLES = [
    ("ai-consulting-cost", "AIコンサルの費用相場【2026年】", "顧問型・プロジェクト型・駐在型…4つの料金体系と適正価格の見極め方。費用を抑える方法も。"),
...
def page_cost():
    ...
    desc = "AIコンサルの費用相場を契約形態別に解説。顧問型は月10〜50万円、プロジェクト型は100万円〜、駐在・常駐型は月100〜300万円が目安。料金表を公開しない会社が多い中で、見積もりで確認すべきポイントと費用を抑える方法も紹介します。"
    faqs = [
...
<section>
  <h2>参考: シクミの料金の考え方</h2>
  <p>当社は「常駐・駐在型」に分類されますが、いきなり月額契約は結びません。<b>無料簡易診断（オンライン・約2時間）で削減効果の目安を先に提示</b>し、実地調査→駐在型支援→AI研修と、各ステップの節目で「次に進むか」を判断できる設計です。料金は業務範囲・稼働時間・体制に応じて個別にお見積りし、納品後のサブスクリプション費用はありません。不要なSaaSの解約提案まで行うため、トータルの固定費はむしろ下がるケースがあります。詳細は<a href="/service/consulting/">サービスページ</a>をご覧ください。</p>
</section>
...
"""
    ...
    write_page(f"/column/{slug}/", hd + article_frame(url, 'AIコンサルの費用相場<br class="sp">——4つの料金体系と見極め方', "AIコンサルを検討して最初に困るのが「料金の目安が公開されていない」こと。この記事では、契約形態別の費用相場と、金額の高低よりも大事な見極めポイントを、実際に支援を行っている当社が解説します。", inner, "無料簡易診断の申込（費用相場記事）", related) + footer())
...
<div class="conclusion">
  <p class="c-label">CONCLUSION</p>
  <p><b>結論:</b> AI導入支援会社は<b>「総合コンサル型」「AI開発型」「研修型」「伴走実装型」の4タイプ</b>に分かれます。IT専任者のいない中小企業に合うのは、現場に入って実装までやる<b>伴走実装型</b>。選ぶときは「誰が手を動かすか」「効果試算があるか」「契約に終わりがあるか」「ツール中立か」「料金体系を説明できるか」の5点を確認してください。</p>
</div>
...
<section>
  ...
  <ol>
    ...
    <li><b>料金体系を説明できるか</b>——「何に・どの単位で費用がかかるか」「契約はいつ終わるか」を商談の場で明確に説明できる会社は、価格の妥当性も検証しやすい。曖昧な回答しか返らない会社は避けてください</li>
  </ol>
</section>
...
<section>
  ...
  <p>株式会社シクミは伴走実装型の中でも、コンサルタントが現場に駐在して部門横断で実装まで行う「駐在型」です。無料簡易診断で効果目安を提示し、実地調査→駐在支援→研修（<a href="/subsidy/">助成金</a>対応）と各段階で継続判断できる設計で、料金は業務範囲と体制に応じて個別にお見積りします。詳しくは<a href="/service/consulting/">サービスページ</a>へ。</p>
</section>
...
"""
    related = [
        ("AIコンサルの費用相場", "4つの料金体系と適正価格の見極め方。", "/column/ai-consulting-cost/"),
...
<section>
  ...
  <p>当社が支援した東京都の不動産管理会社では、物件書類の検索・転記が手作業で、表記揺れによる重複登録が運用工数を約1.2倍に膨らませていました。書類の自動リネーム・名寄せ・1物件1ページ帳票の自動生成・基幹システム入力データの自動生成を構築した結果、<b>書類検索・転記の工数を月40時間削減</b>。さらに「転記先」だった月額基幹システム自体が不要と判明し、解約で固定費も削減しました（現地訪問5回・約2ヶ月）。</p>
